...
「加入圖片」積木及「更換圖片」積木可以控制圖片的來源網址，決定展示哪一張圖片，並且能夠將圖片的名稱命名為變數，藉由變數的命名讓後面的積木輕鬆控制圖片的樣式和互動。
...
> - 若不使用「圖片定位」積木，圖片的預設位置：
    ...
    - 例如 Webduino 圖片的大小為 400*400，那預設座標就會是 (200,300)
...
- 「圖片尺寸變化」積木：若設定圖片*尺寸變大 100 點*，會顯示圖片寬度為*當下寬度加上 100 點*。
...
先任意對圖片做調整到想要的樣式，接著在後方放入「小怪獸講話」積木，放入想讓小怪獸說出的圖片數值。  
...
「顯示隱藏圖片」積木和「圖片透明度」積木不同，是直接將設定的圖片顯示或隱藏，選單內可以選擇顯示或隱藏圖片。
